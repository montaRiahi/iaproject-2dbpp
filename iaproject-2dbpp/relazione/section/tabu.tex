...
Il \textit{target bin} é il bin che si presume sia più facile da svuotare; dato un insieme di bin, il bin cercato é quello che minimizza il risultato della seguente funzione (\textit{filling function}), calcolata quindi per ciascun bin:
\[
...
\item influisce sulla velocità di esecuzione dell'algoritmo infatti, sia $B$ l'insieme di bin, i $k$ bin possono essere presi in $\binom{|B|-1}{k}$ modi (viene escluso dai possibili bin il \textit{bin target});
\end{enumerate}
...
\texttt{\footnotesize
   ...
   \>for each $k-$tupla K di bins escluso quello target t do\\
   ...
   \>\>\>\>endif\\
   \>\>\> A(S) = k+1 e k>1\\
   \>\>\>\> I = insieme di k+1 bins usati da A\\
